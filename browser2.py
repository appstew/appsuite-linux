...
tablistpath = os.path.join(path, 'tablist')
myiconpath = os.path.join(path,'source/images')

copyFolder(os.path.join(path, 'source/tablist'), os.path.join(path, 'tablist'))
...
class outerWidget(QWidget):
    def __init__(self):
        ...
        self.btn.pressed.connect(lambda: self._load(tabFileList) & print("button pressed") & self.btn.deleteLater())
...
class browserWidget(QWidget):
    ...
    def open_tabs_from_file(self, tabWidget, filepath, urlbar):
        ...
        self.add_new_tab(tabWidget, QUrl("chrome://gpu/"), urlbar)
